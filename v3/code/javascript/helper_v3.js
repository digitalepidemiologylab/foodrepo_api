...
function show_products(data) {
  ...
  for (let i = 0, n = data.length; i < n; i++) {
    let product = data[i];
    let product_name = product.name ? product.name : "";
    descriptions += 'id: ' + product.id + ", barcode: " + product.barcode + ', name: "' + product_name + '"<br>';
  }
...
function process_product_data(data, current_endpoint) {
  ...
  if (count === 0 && data.length > 1) {
    ...
    let search_terms = {barcode : [product0.barcode, product1.barcode]};
